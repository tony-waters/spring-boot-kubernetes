...
echo "Reset complete."
echo "Next steps:"
echo "  1) kind create cluster --image kindest/node:v1.33.4"
echo "  2) docker run -d --name ${CCM_NAME} --rm --network host -v /var/run/docker.sock:/var/run/docker.sock registry.k8s.io/cloud-provider-kind/cloud-controller-manager:v0.10.0"
echo "  3) kubectl get gatewayclass"
echo "  4) helm upgrade --install ${RELEASE_NAME} ./helm"